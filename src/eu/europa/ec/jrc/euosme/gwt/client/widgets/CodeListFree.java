...
public class CodeListFree extends Composite  {
	...
	public CodeListFree(String label, String help, boolean required, String myListName, String myDefaultValue, CheckFunctions check, boolean showOracleList, boolean order) {
		...
		myTextBox.getValueBox().addFocusHandler( new FocusHandler(){
			@Override
			public void onFocus(FocusEvent event) {
				// select the related tree item
				if (myTreeItem==null) {
					TreeItem val = null;
					val = Utilities.getSelectTreeItem(myTextBox.getValueBox().getName());
					myTreeItem = val;						
				}
				// set selected item
				if (myTreeItem!=null) {
		...
		myTextBox.getValueBox().addKeyPressHandler(new KeyPressHandler() {			
			@Override
			public void onKeyPress(KeyPressEvent event) {
				if (event.getNativeEvent().getKeyCode() == KeyCodes.KEY_TAB) {
			    	myCheck();
			    	if (myError.isVisible()) {
			    		myTextBox.getValueBox().cancelKey();
			    		myTextBox.setFocus(true);
			    	}
			    }
			}
		...
		myTextBox.getValueBox().addKeyUpHandler(new KeyUpHandler() {
		...
		myTextBox.getValueBox().addBlurHandler(new BlurHandler() {
			@Override
	...
	public void setFormName(String name) {
		myTextBox.getValueBox().setName(name);
