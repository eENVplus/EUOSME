...
public class MD_Keywords_eENVplus extends CI {
	...
	HorizontalPanel ENVPanel = new HorizontalPanel();
	public Label keywordENVObj = new Label();
	public Label sourceENVObj = new Label();
	public Label uriENVObj = new Label();
		
	/** Filter */
	HorizontalPanel filterPanel = new HorizontalPanel();
	
	/** 
     * constructor MD_Keywords_eENVplus model
     * 
     ...
     */
	public MD_Keywords_eENVplus(String label, boolean required, boolean multiple, String help) {
		...
		final Button filterClearButton = new Button("Reset");
		...
		suggestObj.addOpenHandler(new OpenHandler<TreeItem>() {
			...
			public void onOpen(OpenEvent<TreeItem> event) {
				...
				if (w.getChildCount()==1 && w.getParentItem()!=null) {
					...
					w.getChild(w.getChildCount()-1).getElement().scrollIntoView();					
				}
		...
		// eENVplus Panel
		keywordENVObj.getElement().getStyle().clearMarginLeft();
		keywordENVObj.setText(constants.selectedValue());
		sourceENVObj.setText("");
		sourceENVObj.setVisible(false);
		uriENVObj.setText("");
		uriENVObj.setVisible(false);
		ENVPanel.add(keywordENVObj);
		ENVPanel.add(sourceENVObj);
		ENVPanel.add(uriENVObj);
		ENVPanel.setVisible(false);			
		fieldsGroup.add(ENVPanel);		
		// initialize the list
		// RPC call if requested
		if (EUOSMEGWT.rpcRepository){
			requestListOfRepository(listScheme);
			//Utilities.setSuggestList(MyResources.INSTANCE.repositoryeENVplus().getText(), listScheme);
			//listScheme.setSelectedIndex(0);
			////requestSuggestions(listScheme.getValue(listScheme.getSelectedIndex()).trim(),"");			
		}
		else
			Utilities.setSuggestList(MyResources.INSTANCE.repositoryeENVplus().getText(), listScheme);	
	}
	...
	private void requestSearch(String scheme, String pattern) {
		...
		suggestObj.clear();
		SafeHtml safeHtml = SafeHtmlUtils.fromString(constants.keyword());	
		suggestObj.addItem(safeHtml);
		...
		if (EUOSMEGWT.rpcRepository) {
			...
			ls.invokeGetRESTfulWebService("repository_env", scheme, LocaleInfo.getCurrentLocale().getLocaleName(), pattern, "virtuoso", callback);
			GWT.log(" root Tree Item " + suggestObj.getItem(0).getText(), null);
		}
		else {
	...
	private void requestSuggestions(String repository, String filter) {
		...
		if (EUOSMEGWT.rpcRepository) {
			...
			GWT.log(" root Tree Item " + suggestObj.getItem(0).getText(), null); 
			RESTfulWebServiceProxyAsync ls = RESTfulWebServiceProxy.Util.getInstance();
			ls.invokeGetRESTfulWebService("topmost_env", repository, LocaleInfo.getCurrentLocale().getLocaleName(), filter,"virtuoso", callback);
			GWT.log(" root Tree Item " + suggestObj.getItem(0).getText(), null);
		}
		else {
			Utilities.setSuggestLocal(Utilities.getResourceRepository(listScheme.getItemText(listScheme.getSelectedIndex()).trim()), suggestObj.getItem(0));	
	...
	private void requestSubSuggestions(TreeItem myTreeItem) {		
		...
		RESTfulWebServiceProxyAsync ls = RESTfulWebServiceProxy.Util.getInstance();
		ls.invokeGetRESTfulWebService("narrower_env",listScheme.getValue(listScheme.getSelectedIndex()).trim() , LocaleInfo.getCurrentLocale().getLocaleName(), myTreeItem.getTitle(), "virtuoso", callback);         
		}
	...
	private void requestListOfRepository(ListBox myListRPC) {
		...
		RESTfulWebServiceProxyAsync ls = RESTfulWebServiceProxy.Util.getInstance();
		ls.invokeGetRESTfulWebService("repositories_env", "", LocaleInfo.getCurrentLocale().getLocaleName(), "", "virtuoso", callback);
	}
}