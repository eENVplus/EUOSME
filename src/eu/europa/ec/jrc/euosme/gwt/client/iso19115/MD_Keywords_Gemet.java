...
//				}
	...
	private void requestSearch(String scheme, String pattern) {
		...
		if (EUOSMEGWT.rpcRepository) {
			...
			ls.invokeGetRESTfulWebService("search", scheme, LocaleInfo.getCurrentLocale().getLocaleName(), pattern, "sesame", callback);
			GWT.log(" root Tree Item " + suggestObj.getItem(0).getText(), null);
		}
		else {
	...
	private void requestSuggestions(String repository, String filter) {
		...
		if (EUOSMEGWT.rpcRepository) {
			...
			GWT.log(" root Tree Item " + suggestObj.getItem(0).getText(), null); 
			RESTfulWebServiceProxyAsync ls = RESTfulWebServiceProxy.Util.getInstance();
			ls.invokeGetRESTfulWebService("repository", repository, LocaleInfo.getCurrentLocale().getLocaleName(), filter, "sesame", callback);
			GWT.log(" root Tree Item " + suggestObj.getItem(0).getText(), null);
		}
		else {
			Utilities.setSuggestLocal(Utilities.getResourceRepository(listScheme.getItemText(listScheme.getSelectedIndex()).trim()), suggestObj.getItem(0));	
	...
	private void requestSubSuggestions(TreeItem myTreeItem) {		
		...
		RESTfulWebServiceProxyAsync ls = RESTfulWebServiceProxy.Util.getInstance();
		ls.invokeGetRESTfulWebService("narrower", myTreeItem.getTitle(), LocaleInfo.getCurrentLocale().getLocaleName(), "","sesame", callback);         
    }
	...
	private void requestListOfRepository(ListBox myListRPC) {
		...
		RESTfulWebServiceProxyAsync ls = RESTfulWebServiceProxy.Util.getInstance();
		ls.invokeGetRESTfulWebService("repositories", "", LocaleInfo.getCurrentLocale().getLocaleName(), "", "sesame", callback);
	}
}